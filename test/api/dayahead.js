// api/dayahead.js  (Vercel Serverless Function - CommonJS)
...
module.exports = async (req, res) => {
  ...
  // Health check: ?test=1 returns a simple JSON so you can verify the route works
  if (req.query && String(req.query.test) === "1") {
    return res.status(200).json({ ok: true, message: "dayahead alive" });
  }

  const token = process.env.ENTSOE_TOKEN;
  if (!token) return res.status(500).json({ error: "ENTSOE_TOKEN not set" });
  ...
  try {
    // Use Node 18+/20 global fetch (no dependency needed)
      ...
      for (const ts of tsArr) {
        ...
        for (const p of periods) {
          ...
          points.forEach((pt, idx) => {
            ...
            if (Number.isFinite(raw)) series.push({ ts: hour.toISOString(), price: raw });
          });
        }
      }
    ...
    return res.status(500).json({ error: "ENTSO-E fetch failed", details: String(e.message || e) });
  }
};
